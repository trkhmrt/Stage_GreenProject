...
def fix_product_model_matching():
    """Fix product model matching to be more accurate"""
    ...
    try:
        ...
        print("🔧 Ürün model eşleştirmeleri düzeltiliyor...")
        ...
        # Define more specific phone models and their years
        phone_models = {
            # iPhone models - more specific matching
        ...
        # Update products with correct model and year data
        updated_count = 0
        ...
        for idx, product in products_df.iterrows():
            ...
            # Find matching model (more specific matching)
            matched_model = None
            ...
            if matched_model:
                ...
                if current_model != matched_model['model'] or current_year != matched_model['year']:
                    # Update the product
                    update_query = """
                    UPDATE products 
                    SET product_model = :model, product_model_year = :year
                    ...
                    with product_engine.connect() as conn:
                        conn.execute(text(update_query), {
                            ...
                            'year': matched_model['year'],
                            'product_id': product['product_id']
                        })
                        conn.commit()
                    
                    print(f"✅ Düzeltildi: {product_name}")
                    print(f"   Eski: {current_model} ({current_year})")
                    print(f"   Yeni: {matched_model['model']} ({matched_model['year']})")
                    updated_count += 1
                else:
                    print(f"✅ Zaten doğru: {product_name} -> {current_model} ({current_year})")
            else:
                print(f"⚠️  Eşleşme bulunamadı: {product_name}")
        
        print(f"\n🎉 Toplam {updated_count} ürün düzeltildi!")
